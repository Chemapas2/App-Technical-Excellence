...
def get_asset_paths():
    return {k: get_asset_path(v) for k, v in ASSET_CANDIDATES.items()}
...
def display_corporate_header(asset_paths):
    ...
    with col1:
        if asset_paths.get("nutreco"):
            st.image(str(asset_paths["nutreco"]), width=250)
    with col2:
        if asset_paths.get("techteam"):
            st.image(str(asset_paths["techteam"]), width=340)
    st.markdown(
        "<div style='margin-top:6px;font-size:30px;font-weight:800;color:#143b8f;'>Assessment técnico · Ranking y reporte corporativo</div>",
...
def find_classification_sheet(wb):
    preferred = ["ENLACES DATOS", "A BASE DE DATOS", "BASE DE DATOS", "BBDD"]
    by_norm = {normalize_for_match(s): s for s in wb.sheetnames}
    for name in preferred:
        key = normalize_for_match(name)
        if key in by_norm:
            return wb[by_norm[key]]
    for s in wb.sheetnames:
        n = normalize_for_match(s)
        if "base" in n or "bbdd" in n or "datos" in n:
            return wb[s]
    return None


def extract_excel_classification(wb):
    ws = find_classification_sheet(wb)
    if ws is None:
        return {"label": "-", "rank": None, "ratio": None, "source": "No disponible"}

    ratio_value = safe_float(ws["K37"].value)
    q_values = [safe_float(ws[f"Q{r}"].value) for r in range(56, 62)]
    q_flags = [int(v) for v in q_values if v is not None]
    rank_value = int(sum(q_flags)) if q_flags else safe_float(ws["M44"].value)
    rank_value = int(rank_value) if rank_value is not None else None

    label_lookup = {}
    for r in range(56, 62):
        rank_cell = safe_float(ws[f"O{r}"].value)
        label_cell = ws[f"P{r}"].value
        if rank_cell is not None and label_cell not in (None, ""):
            label_lookup[int(rank_cell)] = str(label_cell).strip()

    label_value = label_lookup.get(rank_value) if rank_value is not None else None
    if not label_value:
        label_value = ws["N44"].value
    label_value = str(label_value).strip() if label_value not in (None, "") else "-"
    ...
    for r in range(56, 62):
        ...
        thresholds.append({
            ...
            "Corte": cutoff,
            "Q": q_flag,
            "Ranking": int(rank_cell) if rank_cell is not None else None,
            "Etiqueta": str(label_cell).strip() if label_cell not in (None, "") else "",
        })
...
def parse_candidate(uploaded_file):
    ...
    excel_profile = extract_excel_classification(wb)
    ...
    return {
        ...
        "global_level": excel_profile["label"],
        "profile_rank": excel_profile["rank"],
        "profile_ratio": excel_profile["ratio"],
        "profile_sheet": excel_profile["source"],
        "profile_thresholds": excel_profile["thresholds"],
        ...
        rows.append(
            {
                ...
                "Ranking Excel": candidate.get("profile_rank") if candidate.get("profile_rank") is not None else "-",
...
st.title("Selector y reporte técnico corporativo")
st.caption(
...
for item in parsed_candidates:
    ...
    with st.expander(title, expanded=(candidate["name"] == selected_candidate["name"])):
        ...
        with right:
            ...
            if candidate.get("profile_ratio") is not None:
                st.caption(f"Ratio K37: {candidate['profile_ratio']:.2%} · ranking Excel: {candidate.get('profile_rank', '-')} · hoja: {candidate.get('profile_sheet', '-')}")
            if candidate.get("profile_thresholds"):
                thr_df = pd.DataFrame(candidate["profile_thresholds"])
